...
module.exports = {
    ...
    async execute({ inter, client }) {
        ...
        if (!filter) return inter.reply({ content: `This filter doesn't exist ${inter.member}\n${actualFilter ? `Filter currently active ${actualFilter}.\n` : ''}List of available filters ${filters.map(x => `**${x}**`).join(', ')}.`, ephemeral: true });
        ...
        inter.reply({ content: `The filter ${filter} is now **${queue.getFiltersEnabled().includes(filter) ? 'enabled\n*this may take a sec...*' : 'disabled'}** ` });
    },
};